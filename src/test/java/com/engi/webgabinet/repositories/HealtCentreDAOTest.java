...
@Tag("DAO")
public class HealtCentreDAOTest {
    @MockBean
    ...
    public void setUp() throws Exception {
        ...
        this.healthCentreDAO = new HealthCenterDAOImpl(this.sessionFactory);
    }

    @Test
    public void should_returnEmptyList_forEmptyrepo() {
        Query query = Mockito.mock(Query.class);
        Mockito.when(this.session.getNamedQuery("findAllHealthCenter")).thenReturn(query);
        Mockito.when(query.getResultList()).thenReturn(new ArrayList());

        List list = healthCentreDAO.findAll();
